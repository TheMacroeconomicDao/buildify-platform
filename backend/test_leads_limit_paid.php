...
// Найдем платный тариф Starter с max_leads = 15
$starterTariff = App\Models\Tariff::where('name', 'Starter')->first();
if (!$starterTariff) {
    echo 'Starter tariff not found' . PHP_EOL;
    exit(1);
}

echo 'Using tariff: ' . $starterTariff->name . ' (max_leads: ' . $starterTariff->max_leads . ', price: ' . $starterTariff->price . ')' . PHP_EOL;

// Назначим Starter тариф пользователю
$executor->update([
    'current_tariff_id' => $starterTariff->id,
    'subscription_started_at' => now(),
    'subscription_ends_at' => now()->addDays(30),
]);

echo 'Assigned Starter tariff to executor' . PHP_EOL;
...
echo 'Max leads allowed: ' . $starterTariff->max_leads . PHP_EOL;
