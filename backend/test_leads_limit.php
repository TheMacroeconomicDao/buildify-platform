...
// Найдем тариф с max_leads = 0
$zeroLimitsTariff = App\Models\Tariff::where('max_leads', 0)->first();
if (!$zeroLimitsTariff) {
    echo 'No tariff with max_leads=0 found' . PHP_EOL;
    exit(1);
}

echo 'Using tariff: ' . $zeroLimitsTariff->name . ' (max_leads: ' . $zeroLimitsTariff->max_leads . ')' . PHP_EOL;

// Назначим этот тариф пользователю
$executor->update([
    'current_tariff_id' => $zeroLimitsTariff->id,
    'subscription_started_at' => now(),
    'subscription_ends_at' => now()->addDays(30),
]);

echo 'Assigned zero limits tariff to executor' . PHP_EOL;
...
echo 'Max leads allowed: ' . $zeroLimitsTariff->max_leads . PHP_EOL;
